var bp="http://localhost:8060/yc27808pro002ssm";
function openPage(url, title) {
	width = 460;
	height = 300;
	t = (window.screen.height - height) / 2;
	l = (window.screen.width - width) / 2;
	open(bp+"/"+url, title, "top=" + t + ",left=" + l + ",width=" + width + ",height="
			+ height);
}
function del(id, name) {
	if (window.confirm("确认要删除：" + name)) {
		location.href = bp+'/user/del/' + id;
	}
}
function checkId(id) {
...
function nameExists(name) {
	...
	var url = bp+"/user/nameExists/"+name+"?t="
			+ Math.random();
	// alert(url);
